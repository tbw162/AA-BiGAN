...
if(opt.dataset == 'CIFAR'):
    from un_dataset.cifarset import create_loader 
    train_pos, val_loader, test_loader = create_loader(opt,kwargs)
elif(opt.dataset == 'F-MNIST'):
    from un_dataset.fmnistset import create_loader
    train_pos, val_loader, test_loader = create_loader(opt,kwargs)
else:
    from un_dataset.mnistset import create_loader
    train_pos, val_loader, test_loader = create_loader(opt,kwargs)
...
for epoch in range(opt.n_epochs):
    ...
    dxx_list = []
    dxz_list = []
    for idx, (image, target) in enumerate(train_pos):
        discriminator.train()
        generator.train()
        encoder.train()
        ...
        cycle_loss = adversarial_loss(discriminator(img_pos,img_pos,'xx')[0],c*valid)+adversarial_loss(discriminator(img_pos,gen_imgs_real,'xx')[0],c*valid)
       
        
       
        g_loss = adversarial_loss(discriminator(gen_imgs_fake,z_out_fake,'xz')[0], c*valid)+(1/2)*cycle_loss
        ...
        e_loss = adversarial_loss(discriminator(pos_imgs,z_out_real,'xz')[0],c*valid)+(1/2)*cycle_loss
...
auc_re.to_csv(PACK_PATH+opt.dir+opt.name+str(opt.normal_digit)+".csv")
